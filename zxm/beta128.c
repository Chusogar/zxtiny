...
// Type I commands — based on ESPectrum state machine (instant completion)
...
static void exec_read_address(Beta128* beta) {
    ...
    beta->drq = true;
    beta->busy = true;
    beta->status = WD_ST_BUSY;  // DRQ resolved dynamically
}
...
static void exec_command(Beta128* beta, uint8_t cmd) {
    {
        ...
        if (dbg_cmd < 100) {
            fprintf(stderr, "[WD1793-DBG] CMD 0x%02X busy=%d\n", cmd, beta->busy);
            dbg_cmd++;
        }
    }
    ...
    if ((cmd & 0xF0) == 0xD0) {
        ...
        if (beta->busy) {
            // Abort current command (ESPectrum: clear busy/drq/state)
            beta->busy = false;
            beta->drq = false;
            beta->data_ptr = NULL;
            beta->data_pos = 0;
    ...
    // Type I commands complete even without a disk — NOTREADY is resolved
    // dynamically on status read (matches real WD1793 and ESPectrum behavior)
    if ((cmd & 0x80) == 0x00) {
        beta->busy = true;
    ...
    switch (port & 0xFF) {
        case 0x1F: {
            ...
            if (beta->status_type1) {
                ...
                // Drive is always "ready" (ESPectrum convention — motor always on)
                if (drv->disk.inserted && drv->disk.write_protected)
                    st |= WD_ST_WRPROTECT;
 
                if (drv->current_track == 0)
                    st |= WD_ST_TRACK0;
 
                // Index pulse (bit 1) — simulated by tick
                if (beta->index_pulse)
                    st |= WD_ST_INDEX;
                ...
                if (beta->drq)
                    st |= WD_ST_DRQ;
                // NOTREADY only for Type II/III when no disk (can't read/write)
                if (!drv->disk.inserted)
                    st |= WD_ST_NOTREADY;
            }
 
            {
                static int dbg_st = 0;
                if (dbg_st < 500) {
                    fprintf(stderr, "[WD1793-DBG] STATUS read=0x%02X pc=0x%04X idx=%d cnt=%d\n",
                            st, beta->dbg_pc, beta->index_pulse, beta->index_counter);
                    dbg_st++;
                }
            }
            return st;
        ...
        case 0xFF: {
            ...
            if (beta->drq) val |= 0x40;
            {
                static int dbg_ff = 0;
                if (dbg_ff < 50) {
                    fprintf(stderr, "[WD1793-DBG] SYSREG $FF=0x%02X intrq=%d fintrq=%d drq=%d\n",
                            val, beta->intrq, beta->fintrq, beta->drq);
                    dbg_ff++;
                }
            }
            return val;
...
void beta128_tick(Beta128* beta, int delta) {
    ...
    bool old_pulse = beta->index_pulse;
    ...
    if (old_pulse != beta->index_pulse) {
        static int dbg_idx = 0;
        if (dbg_idx < 20) {
            fprintf(stderr, "[IDX-DBG] pulse %d->%d cnt=%d delta=%d\n",
                    old_pulse, beta->index_pulse, beta->index_counter, delta);
            dbg_idx++;
        }
    }
}