...
typedef struct {
 ...
 bool     vic_display_active;
 uint16_t vic_vc;
 uint16_t vic_vc_base;
 uint8_t  vic_rc;
 bool     vic_bad_line;
 uint16_t vic_char_base;
 uint16_t vic_screen_base;
 uint16_t vic_bitmap_base;
 uint8_t  vic_border_color;
 uint8_t  vic_bg_color[4];
 ...
 float audio_buffer[C64_SAMPLES_PER_FRAME * 2];
 int   audio_pos;
